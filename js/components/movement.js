...
export function animation(el, tanks) {
  if (tanks.life === true) {
    ...
    const classAll = Array.from(classAlls).filter(
      (e) => e.classList !== undefined
    );

    const filter = (el, className) =>
      classAll.forEach((e) => e.classList.contains(className) && el.push(e));
...
function movementEnemyUp(tank) {
  ...
  let up = tank.pozition.top;
  pozMapAll(tank, true);
  if (tank.stopEnemiLeft === false || tank.stopEnemiRight === false) {
    randomDesc(tank);
  }
  if (tank.stopEnemiUp === true) {
...
function movementEnemyLeft(tank) {
  ...
  let left = tank.pozition.left;
  pozMapAll(tank, true);
  if (tank.stopEnemiUp === false || tank.stopEnemiDown === false) {
    randomDesc(tank);
  }
  if (tank.stopEnemiLeft === true) {
...
function pozMapAll(tank, enemy) {
  ...
  if (enemy === true) {
    tank.stopEnemiUp = tank.pozitionMapTop.some((e) => {
      if (stopMovement.includes(e)) {
        if (e === 2) {
          pozMapAll(tank);
          // console.log("Элемент равен 2 вверху");
        }
        return true;
      }
      return false;
    ...
    tank.stopEnemiDown = tank.pozitionMapDown.some((e) => {
      if (stopMovement.includes(e)) {
        if (e === 2) {
          pozMapAll(tank);
          // console.log("Элемент равен 2 внизу");
        }
        return true;
      }
      return false;
    ...
    tank.stopEnemiLeft = tank.pozitionMapLeft.some((e) => {
      if (stopMovement.includes(e)) {
        if (e === 2) {
          pozMapAll(tank);
          // console.log("Элемент равен 2 влево");
        }
        return true;
      }
      return false;
    ...
    tank.stopEnemiRight = tank.pozitionMapRight.some((e) => {
      if (stopMovement.includes(e)) {
        if (e === 2) {
          pozMapAll(tank);
          // console.log("Элемент равен 2 вправо");
        }
        return true;
      }
      return false;
...
function randomDesc(tank, desc) {
  ...
  const dx = player.pozitionMap.left - tank.pozitionMap.left;
  const dy = player.pozitionMap.top - tank.pozitionMap.top;
